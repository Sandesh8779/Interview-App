...
// Read the JSX content from separate files to avoid parser issues
const srcJSX = `
    ...
    <section className="panel">
      ...
      <div className="cd-result-panel">
        ...
        <div className="cd-result-scores">
          <div className="cd-score-card"><h5>MCQ Test</h5><div className="cd-score-value">{avgScore >= 5 ? avgScore : "--"}/10</div>
          <div className="cd-score-card"><h5>Coding Test</h5><div className="cd-score-value">{avgScore >= 5 ? avgScore : "--"}/10</div>
          <div className="cd-score-card"><h5>Interview</h5><div className="cd-score-value">{avgScore}/10</div>
        </div>
        {interview.feedback && (
          <div className="cd-feedback-box"><h4>Interviewer Feedback</h4><p>{interview.feedback}</p></div>
        )}
        {avgScore >= 7 && (
...
const hrAiJSX = `
function HrAiPanel() {
  const [messages, setMessages] = useState([{ from: "ai", text: "Tell me about yourself and your recent project." }]);
  ...
  function send(e) {
    ...
    setIsTyping(true);
    const responses = ["That is a great experience! Can you tell me about a challenge you faced?", "Interesting. What technologies did you use in that project?", "How do you handle working under pressure?", "Where do you see yourself in 5 years?", "Thank you for sharing. Do you have any questions for me?"];
    setTimeout(() => { setMessages(prev => [...prev, { from: "ai", text: responses[Math.floor(Math.random() * responses.length)] }]); setIsTyping(false); }, 1500);
  }
  const suggestedQs = ["Tell me about your skills", "Why do you want this job?", "Describe your teamwork experience", "What are your strengths?"];
  return (
    <section className="panel">
      <h2><MessageSquareText size={20} /> HR AI Interview</h2>
      <div className="cd-ai-avatar">
        <div className="cd-ai-avatar-circle">AI</div>
        <div><strong>InterviewFlow AI Coach</strong><p style={{color:"#b8c0ce",margin:"2px 0 0",fontSize:"0.85rem"}}>Practice your HR interview skills</p></div>
      <div className="chat-panel" style={{maxHeight:280,overflowY:"auto"}}>
        {messages.map((msg, i) => (<div key={i} className={"chat-line " + msg.from}>{msg.text}</div>))}
        {isTyping && (<div className="cd-ai-typing"><span></span><span></span><span></span></div>)}
        <div ref={bottomRef} />
      </div>
      ...
      <div className="cd-ai-suggested">{suggestedQs.map(q => (<button key={q} type="button" onClick={() => setInput(q)}>{q}</button>))}</div>
    </section>
  );
}
`;
      ...
      <div style={{display:"grid",gap:4}}>
        {notifications.map((n, i) => (
          <div key={i} className={"cd-notif-item" + (n.unread ? " unread" : "")}>
            ...
            <div className="cd-notif-body"><strong>{n.title}</strong><p>{n.desc}</p></div>
  ...
  return (
    <section className="panel">
      ...
      <div style={{display:"flex",gap:8,marginBottom:16}}>
        {TASKS.map((t, i) => (<button key={i} className={active===i?"primary":"secondary"} onClick={() => setActive(i)} type="button" style={{padding:"6px 16px",minHeight:36,fontSize:"0.85rem"}}>Task {i+1}</button>))}
      </div>
      {submitted ? (
        <div style={{textAlign:"center",padding:32}}>
          ...
          <div className="cd-output-console" style={{marginTop:16,textAlign:"left"}}>{"> All tasks submitted successfully.\\n> Awaiting interviewer evaluation..."}</div>
      ) : (
        <form className="stack" onSubmit={submit}>
          <div className="cd-coding-split">
            <div className="cd-coding-problem">
              ...
              <div><strong style={{fontSize:"0.85rem",display:"block",marginBottom:6}}>Test Cases</strong>
                {TASKS[active].tests.map((tc, i) => (<div key={i} className={"cd-test-case"+(testResults ? " "+testResults[i] : "")}>{testResults ? (testResults[i] === "pass" ? "PASS" : "FAIL") : "---"} {tc}</div>))}
              </div>
            <div>
              <div className="cd-code-editor-wrapper">
                <div className="cd-code-header"><span>JavaScript</span><div className="cd-code-lang"><span className="active">JS</span><span>Python</span><span>Java</span></div>
                <textarea className="cd-code-editor" value={codes[active]} onChange={e => setCodes(prev => prev.map((c_, i) => i===active ? e.target.value : c_))} spellCheck={false} style={{fontFamily:'"Courier New",monospace',fontSize:"0.88rem",lineHeight:1.6,minHeight:200,padding:14,border:"none",background:"#1d2433",color:"#e4e9f2",resize:"vertical"}} />
              </div>
...
// Write components to separate files to avoid Node.js parsing the JSX
const compDir = __dirname + '/.comp_cache';
if (!fs.existsSync(compDir)) fs.mkdirSync(compDir);
fs.writeFileSync(compDir + '/cand_result.jsx', srcJSX, 'utf8');
fs.writeFileSync(compDir + '/hr_ai.jsx', hrAiJSX, 'utf8');
fs.writeFileSync(compDir + '/notif.jsx', notifJSX, 'utf8');
fs.writeFileSync(compDir + '/coding.jsx', codingJSX, 'utf8');

// Now read App.jsx
const appPath = __dirname + '/src/App.jsx';
let content = fs.readFileSync(appPath, 'utf8');

// These replacements are done using simple string operations  
// They don't need Node to parse the JSX

// Replace HrAiPanel
const hrStart = content.indexOf('function HrAiPanel');
const hrEnd = content.indexOf('function NotificationList', hrStart);
if (hrStart >= 0 && hrEnd >= 0) {
  content = content.substring(0, hrStart) + hrAiJSX.trim() + '\n' + content.substring(hrEnd);
  console.log('Replaced HrAiPanel');
}

// Replace NotificationList
const nStart = content.indexOf('function NotificationList');
const nEnd = content.indexOf('function InterviewTable', nStart);
if (nStart >= 0 && nEnd >= 0) {
  content = content.substring(0, nStart) + notifJSX.trim() + '\n' + content.substring(nEnd);
  console.log('Replaced NotificationList');
}

// Add CandidateResult before InterviewTable
if (!content.includes('function CandidateResult')) {
  const insPos = content.indexOf('function InterviewTable(');
  if (insPos >= 0) {
    content = content.substring(0, insPos) + srcJSX.trim() + '\n\n' + content.substring(insPos);
    console.log('Added CandidateResult');
  }
}

// Replace CodingTest
const cStart = content.indexOf('function CodingTest');
const cEnd = content.indexOf('function HrAiPanel', cStart);
if (cStart >= 0 && cEnd >= 0) {
  content = content.substring(0, cStart) + codingJSX.trim() + '\n' + content.substring(cEnd);
