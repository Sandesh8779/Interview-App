...
// ===== 1. Update CodingTest =====
const oldCodingStart = 'function CodingTest({ setMessage }) {';
const oldCodingEnd = '}\nfunction HrAiPanel() {';
const startIdx = c.indexOf(oldCodingStart);
const endIdx = c.indexOf(oldCodingEnd);

if (startIdx >= 0 && endIdx >= 0) {
  const oldCodingFunc = c.substring(startIdx, endIdx + oldCodingEnd.length);
  
  const newCodingFunc = `function CodingTest({ setMessage }) {
  const TASKS = [
              ...
              <div>
                ...
                {TASKS[active].tests.map((tc, i) => (
                  <div key={i} className={'cd-test-case'+(testResults ? ' '+testResults[i] : '')}>
                    {testResults ? (testResults[i] === 'pass' ? '\\u2713' : '\\u2717') : '\\u25CB'} {tc}
                  </div>
                ))}
              </div>
            <div>
...
function HrAiPanel() {`;
  ...
  c = c.substring(0, startIdx) + newCodingFunc + c.substring(endIdx + oldCodingEnd.length);
  console.log('Updated CodingTest successfully');
} else {
  console.log('Could not find CodingTest boundaries');
  console.log('startIdx:', startIdx, 'endIdx:', endIdx);
}

// ===== 2. Update HrAiPanel =====
const oldHrStart = 'function HrAiPanel() {';
const oldHrEnd = '}\n\nfunction NotificationList() {';
const hrStartIdx = c.indexOf(oldHrStart);
const hrEndIdx = c.indexOf(oldHrEnd);

if (hrStartIdx >= 0 && hrEndIdx >= 0) {
  const oldHrFunc = c.substring(hrStartIdx, hrEndIdx + oldHrEnd.length);
  
  const newHrFunc = `function HrAiPanel() {
...
function NotificationList() {`;
  ...
  c = c.substring(0, hrStartIdx) + newHrFunc + c.substring(hrEndIdx + oldHrEnd.length);
  console.log('Updated HrAiPanel successfully');
}

// ===== 3. Update NotificationList =====
const oldNotifStart = 'function NotificationList() {';
const oldNotifEnd = '}\n\nfunction InterviewTable(';
const notifStartIdx2 = c.indexOf(oldNotifStart);
const notifEndIdx2 = c.indexOf(oldNotifEnd);

if (notifStartIdx2 >= 0 && notifEndIdx2 >= 0) {
  const oldNotifFunc = c.substring(notifStartIdx2, notifEndIdx2 + oldNotifEnd.length);
  
  const newNotifFunc = `function NotificationList({ interviews = [] }) {
  const notifications = [
        ...
        {notifications.map((n, i) => (
          <div key={i} className={'cd-notif-item' + (n.unread ? ' unread' : '')}>
            <div className={'cd-notif-icon ' + n.icon}>
              {n.icon === 'success' ? '\\u2713' : n.icon === 'warning' ? '!' : 'i'}
            </div>
...
function InterviewTable(`;
  ...
  c = c.substring(0, notifStartIdx2) + newNotifFunc + c.substring(notifEndIdx2 + oldNotifEnd.length);
  console.log('Updated NotificationList successfully');
}

// ===== 4. Add CandidateResult component =====
if (!c.includes('function CandidateResult')) {
  // Find the position before "function InterviewTable"
  const insertPos = c.indexOf('function InterviewTable(');
  if (insertPos >= 0) {
    const candidateResultComp = `
function CandidateResult({ interview }) {
  ...
  const verdict = avgScore >= 7 ? 'passed' : avgScore >= 4 ? 'pending' : 'failed';
...
`;
    c = c.substring(0, insertPos) + candidateResultComp + c.substring(insertPos);
    console.log('Added CandidateResult component');
  }
}
